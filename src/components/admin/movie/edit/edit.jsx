...
import { clearStatusEditMovie,editMovie } from '../../../../redux/adminSlice';
import { toast } from 'react-toastify';
import { STATUS_CODE_MOVIE_EDIT_SUCCESS } from '../../../../constants';
import { Dropdown, DropdownToggle, DropdownMenu, DropdownItem } from 'reactstrap';

EditMovie.propTypes = {
    modalEditMovie: PropTypes.bool,
    toggleEditMovie: PropTypes.func,
    movie: PropTypes.object,
};

function EditMovie(props) {
    const {modalEditMovie,toggleEditMovie,movie} = props
    const { register, handleSubmit, formState: { errors },setValue } = useForm();
    const {t} = useTranslation();
    const dispatch = useDispatch()
    const status = useSelector(state => state.admin.movies.statusEdit)
    const notifyEditMovieSuccess = () => toast.success(t('toast.movie.edit_success'));
    ...
    useEffect(() => {
        if(status) {
            toggleEditMovie()
            if(status === STATUS_CODE_MOVIE_EDIT_SUCCESS){
    ...
    function onSubmitEditMovie(data){
    ...
    return (
        <div>
            <Modal isOpen={modalEditMovie} toggle={toggleEditMovie}>
                <form onSubmit={handleSubmit(onSubmitEditMovie)}>
                    <ModalHeader toggle={toggleEditMovie}>{t('admin.movie_info.new_title')}</ModalHeader>
                    <ModalBody>
                        ...
                        <div className="input-item">
                            ...
                            <input {...register("trailer")} type="file" id="input__trailer" src="zing.vn" placeholder={t('movie.input_trailer')}/> 
                        </div>
                        <div className="input-item">
                            <label htmlFor="input__image">{t('movie.image')}</label>  
                            <input {...register("image")} type="file" id="input__image" placeholder={t('movie.input_image')}/> 
                        </div>
                        <div className="input-item">
                    ...
                    <ModalFooter>
                        <Button className="btn__yes" type="submit">{t('admin.movie_info.edit_yes')}</Button>
                        <Button color="secondary" className="btn__no" onClick={toggleEditMovie}>{t('admin.movie_info.edit_no')}</Button>
                    </ModalFooter>
                </form>
            </Modal>
        </div>
